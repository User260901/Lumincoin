...
export class Table {
    ...
    private deleteButtonElement: HTMLElement | null = null;
    private todayButton: HTMLElement | null = null;

    private targetElement: HTMLElement | null = null;
    private eventListenerAtached: boolean = false
    private dateFromData: string | null = null;
    private dateTodata: string | null = null;
    ...
    async getTableByFilter(dateFrom: string | null, dateTo: string | null) {
        ...
        let response: TableResponseType[] | DefaultResponseType = await HttpRequests.request(url)
        if ((response as DefaultResponseType).error !== undefined) {
            throw new Error("Error")
        }

        this.showElements(response as TableResponseType[])
    }
    ...
    showElements(data: TableResponseType[]) {
        if (data) {
            ...
            data.forEach((item, index) => {
                    ...
                    trElement.setAttribute('id', item.id.toString())
                    trElement.style.color = 'black'

                    let celNum = trElement.insertCell()
                    celNum.innerText = (++index).toString()
                    ...
                    let categoryCell: HTMLElement = trElement.insertCell();
                    if (!item.category) {
        ...
        document.querySelectorAll('.remove-icon').forEach(item => {
            item.addEventListener('click', () => {
                const itemElement: HTMLElement | null | undefined = item.parentElement?.parentElement
                if (itemElement) {
                    this.targetElement = itemElement
                }
            })
        })
        ...
        if (this.deleteButtonElement) {
            this.deleteButtonElement.addEventListener('click', async () => {
                if (this.targetElement) {
                    let result:DefaultResponseType = await HttpRequests.request('/operations/' + this.targetElement.id, "DELETE");
                    if (!result.error) {
